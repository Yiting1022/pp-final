...
CSV_COMPARE="${OUT_DIR}/cpu_vs_gpu_comparison.csv"

# 清空舊檔（如果存在）
rm -f "$CSV_PATHS" "$CSV_STEPS" "$CSV_THREADS" "$CSV_COMPARE"

echo "=== CPU Experiments ==="
echo "[1/4] Paths scaling (European / Asian)..."
...
echo "[2/4] Steps scaling (Asian / European)..."
...
for STEPS in "${STEPS_LIST[@]}"; do
  ...
  ./mc_pricer --type european \
    --S0 "$S0_SINGLE" --K "$S0_SINGLE" --r 0.05 --sigma "$SIGMA_SINGLE" --T 1.0 \
    --steps "$STEPS" --paths "$PATHS_SINGLE" --seed 45 --threads "$THREADS" --csv "$CSV_STEPS" > /dev/null
done

echo "[3/4] OpenMP threads scaling (Basket)..."
...
echo "[4/4] CPU vs GPU comparison (European / Asian / Basket)..."
...
for PATHS in "${COMPARE_PATHS_LIST[@]}"; do
  ...
  ./mc_pricer --type european \
    --S0 "$S0_SINGLE" --K "$S0_SINGLE" --r 0.05 --sigma "$SIGMA_SINGLE" --T 1.0 \
    --paths "$PATHS" --seed 50 --threads "$THREADS" --csv "$CSV_COMPARE" > /dev/null

  # Asian
  ./mc_pricer --type asian \
    --S0 "$S0_SINGLE" --K "$S0_SINGLE" --r 0.05 --sigma "$SIGMA_SINGLE" --T 1.0 \
    --steps 252 --paths "$PATHS" --seed 51 --threads "$THREADS" --csv "$CSV_COMPARE" > /dev/null

  # Basket (8 assets)
  ./mc_pricer --type basket \
    --S0 "$S0_MULTI" --K "$S0_MULTI" --r 0.05 --sigma "$SIGMA_MULTI" --T 1.0 \
    --steps 252 --assets 8 --rho "$RHO_MULTI" \
    --paths "$PATHS" --seed 52 --threads "$THREADS" --csv "$CSV_COMPARE" > /dev/null
done
...
echo "  - cpu_vs_gpu_comparison.csv"
